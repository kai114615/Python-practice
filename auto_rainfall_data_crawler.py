...
# 等待雨量連結顯示並點擊進入
try:
    locator = (By.CLASS_NAME, 'cube-head')
except Exception as e:
    print(e)

driver_wait = WebDriverWait(driver, 10).until(
    EC.presence_of_element_located(locator), '找無元素')
rainpage = driver.find_element(By.LINK_TEXT, '雨量')
rainpage.click()

# 進入雨量網頁後連點擊10分鐘資料並進入
rainfall10min = driver.find_element(By.LINK_TEXT, '10分鐘資料')
rainfall10min.click()
...
try:
    ...
    if not os.path.exists('雨量資料'):
        ...
        # new_path = os.chdir(path + '/雨量資料/')
        print(f"資料夾已存在 檔案大小:{os.path.getsize(path)}")
