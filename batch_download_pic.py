...
def download_files(download_year, download_month, download_day):
    for i in range(1,download_day+1): #設定下載幾張
            ...
            with open(str(download_month) + '月CWB每日雨量/' + str(download_year) + str(download_month) + f'{i:02}' + '.png', 'wb') as f:  #圖片檔名及類型設定
                f.write(pic_list.content)

# 自訂顯示狀態函式
def shown_files():
    print(f"資料夾已存在")
    print('檔案內容有:', os.listdir(str(download_month) + '月CWB每日雨量'))

# 自訂完成狀態函式
def finish_status():
    print("下載完畢")
    print(f"圖片張數:{len(os.listdir(str(download_month) + '月CWB每日雨量'))}張")



if os.path.exists(str(download_month) + '月CWB每日雨量'):
    if len(os.listdir(str(download_month) + '月CWB每日雨量')) >= int(download_day):
        shown_files()
    else:
